/* Özel Meşe Ağız ve Diş Sağlığı Polikliniği — site uygulaması.
   Verdant Dental tasarım sistemi (954de0fa) üzerine kurulur. Derleme adımı yok:
   React ve tasarım sistemi paketi window üzerinden gelir. */
(function () {
  'use strict';
  ...
  var DS = window.VerdantDentalDesignSystem_954de0 || {};
  var NavBar = DS.NavBar;
  var Card = DS.Card;
  var Button = DS.Button;
  var Field = DS.Field;
  var Input = DS.Input;
  var Checkbox = DS.Checkbox;
  ...
  function useDar() {
    var pair = useState(function () { return window.innerWidth < DAR_ESIK; });
  ...
  /* Tasarım sistemi paketi yüklenmediyse sessizce boş sayfa vermek yerine haber ver. */
  if (!NavBar || !Card || !Button) {
    document.getElementById('root').innerHTML =
      '<div class="noscript-note"><h1>Sayfa yüklenemedi</h1>' +
      '<p>Tasarım sistemi paketi yüklenemedi. Lütfen sayfayı yenileyin veya ' +
      '<a href="' + KLINIK.telHref + '">' + KLINIK.telefon + '</a> numarasından bize ulaşın.</p></div>';
    return;
  }
